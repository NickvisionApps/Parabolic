...
public class Media
{
    ...
    public Media(JsonElement ytdlp, ITranslationService translator, DownloaderOptions options, string suggestedSaveFolder, string suggestedSaveFilename, int? playlistPosition) : this(suggestedSaveFolder, suggestedSaveFilename, playlistPosition)
    {
        if (playlistPosition.HasValue)
        {
            ...
            if (Url.IsEmpty() && ytdlp.TryGetProperty("webpage_url", out var webpageUrlProperty))
            {
        ...
        else
        {
            ...
            if (Url.IsEmpty() && ytdlp.TryGetProperty("url", out var urlProperty))
            {
                if (Uri.TryCreate(urlProperty.GetString() ?? string.Empty, UriKind.Absolute, out var uri))
                {
                    Url = uri;
        ...
        if (string.IsNullOrEmpty(Title) && ytdlp.TryGetProperty("title", out var titleProperty))
        {
