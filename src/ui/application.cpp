...
Application::Application(const std::string& id, GApplicationFlags flags) : m_adwApp{ adw_application_new(id.c_str(), flags) }
{
    ...
    m_appInfo.setChangelog("<ul><li>Fixed an issue where videos could not be downloaded on ARM64</li><li>Fixed an issue where 'Best' and 'Good' would download the same video quality</li><li>Added translation support</li><li>Added Arabic translation (Thanks @fawaz006!)</li><li>Added Dutch translation (Thanks @Vistaus!)</li><li>Added French translation (Thanks @zothma!)</li><li>Added Russian translation (Thanks @fsobolev!)</li></ul>");
